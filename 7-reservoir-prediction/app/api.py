...
def test():
    ...
    rounded_test_y = np.round(y_test, decimals=4)
    # reshaped_array = rounded_prediction_data.reshape(29642)
    reshaped_array = rounded_test_y.reshape(7452)
    new_list=reshaped_array.tolist()
    rounded_list = [round(num, 4) for num in new_list]
    input_data={"status": "sucess", 'data': rounded_list}
    JSONResponse.content = input_data
    # print(y_test)
    # print(y_test.shape)
...
def predict(response: Response, sample: Reservoir):
    ...
    prediction_data = model.predict(test_x_array)
    rounded_prediction_data = np.round(prediction_data, decimals=4)
    # reshaped_array = rounded_prediction_data.reshape(29642)
    reshaped_array = rounded_prediction_data.reshape(7452)
    new_list=reshaped_array.tolist()
    rounded_list = [round(num, 4) for num in new_list]
